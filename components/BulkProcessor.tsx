...
import { generateTemplateCsvUrl } from '../utils/csv';
import { SparklesIcon, FileTextIcon, ResetIcon, LoadingSpinner, UploadIcon } from './icons';
import type { BulkProduct, CsvProduct } from '../types';
...
export const BulkProcessor: React.FC<{
                                        ...
                                        <td style={{...tdStyle, textAlign: 'center'}}>
                                            {p.status === 'completed' && (
                                                <input
                                                    ...
                                                    aria-label={`Mark ${(p.source as CsvProduct).sku} as reviewed`}
                                                />
                                            )}
                                        </td>
                                    )}
    ...
    return (
        <div style={{ backgroundColor: '#1F2937', padding: '2rem', borderRadius: '0.75rem', textAlign: 'center' }}>
            <h2 style={{ marginTop: 0 }}>Bulk Create New Products</h2>
            ...
            <div
                ...
                onClick={() => document.getElementById('csv-input-create')?.click()}
            ...
            <input type="file" id="csv-input-create" accept=".csv" onChange={e => handleFileChange(e.target.files)} style={{ display: 'none' }} />
            <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem' }}>
                <a href={generateTemplateCsvUrl()} download="template-create.csv" style={{display: 'inline-flex', alignItems: 'center', textDecoration: 'none', padding: '0.5rem 1rem', backgroundColor: '#374151', color: '#F9FAFB', border: 'none', borderRadius: '0.375rem', fontWeight: 600}}>
